...
function set-window-title {
  ...
  zstyle -s ':prezto:module:konsole:window-title' format 'title_format' || title_format="%s"
  zformat -f title_formatted "$title_format" "s:$argv"

  if [[ "$TERM" == screen* ]]; then
    title_format="\ek%s\e\\"
  else
    title_format="\e]0;%s\a"
  fi
...
function set-tab-title {
  local title_format{,ted}
  zstyle -s ':prezto:module:konsole:tab-title' format 'title_format' || title_format="%s"
  zformat -f title_formatted "$title_format" "s:$argv"

  printf "\e]30;%s\a" ${(V%)title_formatted}
}
...
if zstyle -t ':prezto:module:konsole' auto-title \
  && ( ! [[ -n "$STY" || -n "$TMUX" ]] )
then
